...
本次实验通过一系列限制操作符与操作数的编程谜题，显著提高了我对于计算机系统底层数据表示与处理的理解水平。在实验前，我仅能对数值进行基本的位运算，但在本次实验后，我掌握了如何利用位运算来构造复杂的程序逻辑，实现了从**用户视图**到**机器视图**的认知转变。

实验过程中的主要挑战是操作符和操作数的严格限制，这使得许多在高级语言中看似简单的逻辑，例如条件判断或循环计数，必须通过精巧的位级操作序列来实现。这种**束手束脚**的编程体验，让我体会到了高级语言**抽象**的意义与价值。如果没有既成的逻辑封装，我将始终在这样的机器级视图下工作，开发难度大幅提升。

为在限制条件下达成目标，**掩码**的构造与使用成为了贯穿整个实验的核心技术。通过设计特定的掩码，我可以实现对任意比特位的提取、设置或屏蔽，从而将注意力集中于需要处理的数据位上，有效降低了问题复杂度。例如在三个浮点数相关函数 (`floatAbsVal`, `floatScale1d2`, `floatFloat2Int`) 中，通过掩码，我得以将浮点数的 `sign`、`exp` 和 `frac` 三个部分分离出来，分而治之，从而将复杂的问题简化为若干个独立的子问题。通过掩码的构造与使用，我对于机器视图有了更深刻的理解，认识到了**计算机系统里的一切都是比特**。

此外，实验也加深了我对**算法思维本质**的理解。例如 `howManyBits` 函数，其难点在于如何在有限的操作数内找到一个数的最高有效位。最初我的线性扫描思路因操作数过多而不可行，最终我通过借鉴**二分查找**思想，将问题逐层分解，才得以在规定限制内解决。这一过程让我认识到了算法并非**固化的代码模板**，而是一种可灵活应用于不同场景的**问题求解思路**。

对于浮点数的处理，关键在于**将问题分解**。通过将浮点数拆解为 `sign`、`exp` 和 `frac` 三个独立部分，我得以分别处理各部分的逻辑，最后再将其合并，从而将复杂的问题加以简化，使得问题迎刃而解。这也让我认识到了浮点数与整数**在设计哲学上的根本差异**：两者是在不同历史背景下为解决不同问题而设计的独立体系，而非简单的扩展或映射关系，这是我能够掌握浮点数表示的关键。

总而言之，本次实验不仅是对位运算技巧的一次集中训练，更是一次深入理解计算机系统内部工作原理的实践。通过在位级层面重构复杂逻辑，我深刻体会了数据表示的精妙、抽象的价值以及算法思维的普适性。整个过程，从纸上演算逻辑、编写代码到调试验证，构成了一个完整的、严谨的工程思维训练闭环。