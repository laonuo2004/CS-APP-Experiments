...
| 参数 | unsigned |
| 功能实现 | 计算f的绝对值的位级表示 |
| 要求 | 可以使用任意整数/无符号数操作，包括 \|\|, &&, if, while 等，将结果返回。 |

**分析：**

函数要求计算 f 的绝对值的位级表示，且当 f 为 NaN 时，返回 f 本身。

对于非 NaN 的情况，得到其绝对值的位级表示的方法十分简单，即将符号位变为0即可，这可以通过与掩码 `0x7fffffff` 进行 `&` 操作实现，且我们可以通过 `~(0x01 << 31)` 得到该掩码。

而对于 NaN 的情况，我们直接返回 f 本身即可，不需要对其进行任何操作，但关键的难点在于如何判断 f 是否为NaN。
...
对于条件一，我们可以通过 `exp = (uf >> 23) & 0xff)` 取出其 8 位阶码字段 (前24位全为0)，并通过 `!(exp ^ 0xff)` 判断其是否全为1。

对于条件二，我们可以通过 `uf << 9` 将符号位与阶码字段移除，此时末尾补充的 9 位小数均为0，因此 f 小数部分不为 0 等价于 `uf << 9` 不为0。

因此我们可以通过这两个条件的与运算来判断 f 是否为 NaN。考虑到条件二的操作数量较少，因此我们将其放在左侧，这在一般可以提前终止的情况下能够减少操作数。

因此最终我们用于判断 f 是否为 NaN 的条件为：`(uf << 9) && !(~((uf >>23) & 0xff))`。

最终函数的实现可以参考 `函数实现` 部分。经统计，操作数为 9，不超过最大操作数10，符合要求。
...
函数要求计算 0.5*f 的位级表示，且当 f 为 NaN 时，返回 f 本身。
...
对于情况二，无穷乘以0.5仍然是无穷，因此我们直接返回 f 本身即可。这可以与情况一合并。

对于情况三，直接右移尾数即可，需要注意的是浮点数默认舍入为向偶数舍入，因此我们需要额外处理一下这一问题。

对于情况四，由于此时 f * 0.5 将会得到非规格化数，因此我们可以提前先将 f 转换为非规格化数，即将 `exp` 置0，并将隐含的1下方到 `frac` 中，随后的处理与情况三相同。

对于情况五，指数减1即可。

最终函数的实现可以参考 `函数实现` 部分。经统计，操作数为 27，小于最大操作数30，符合要求。
...
| 参数 | unsigned |
| 功能实现 | 计算(int) f的位级表示 |
| 要求 | 可以使用任意整数/无符号数操作，包括 \|\|, &&, if, while 等，将结果返回。超出范围（包括 NaN 和无穷大）应返回 0x80000000u。 |

**分析：**

函数要求将浮点数的位级表示转换为整数，即实现 `(int) f` 的功能。本题需要考虑四种情况。

1. NaN 或无穷大

当指数全为1时，该浮点数为 NaN 或无穷大，根据题目要求，需要返回 `0x80000000u`。

2. 数值太小

浮点数的实际指数为 `E = exp - 127`。当 `E < 0` 时，说明该浮点数的绝对值小于1，转换为整数时会被截断为0。

3. 数值太大

int 类型的范围为 $[-2^{31}, 2^{31}-1]$，当 `E >= 31` 时，该浮点数的绝对值至少为 $2^{31}$，已经超出int范围，需要返回 `0x80000000u`。

4. 正常转换

  对于正常情况，我们可以按照如下4个步骤进行处理：

  1. 构造完整的尾数值：`value = frac | (0x01 << 23)`（加上隐含的1位）
  2. 根据指数 `E` 调整位置：
    - 若 `E < 23`：说明小数点在尾数中间，需要右移 `(23-E)` 位
    - 若 `E >= 23`：说明小数点在尾数右侧，需要左移 `(E-23)` 位

  3. 检查溢出：
    - 正数的最大值为 $2^{31}-1$，因此 `value` 不能大于 `0x80000000u`
    - 负数的最大值为 $2^{31}$，因此 `value` 可以等于 `0x80000000u`
    - 综合判断条件为：`value > 0x80000000u || (value == 0x80000000u && !sign)`

  4. 应用符号：最后据符号位决定返回正值还是负值

最终函数的实现可以参考 `函数实现` 部分。经统计，操作数为 20，小于最大操作数30，符合要求。
...
本次实验完成了15个位运算及浮点数操作函数的设计与实现，涵盖了位级运算、补码表示以及IEEE 754浮点数标准等核心内容。通过严格的操作数限制，实验强化了对底层数据表示与位操作的理解。

在实验过程中，二分查找算法的应用构成了主要难点。`howManyBits` 与 `intLog2` 两个函数均需要在不使用循环结构的前提下定位最高有效位，传统的逐位遍历方法会导致操作数超出限制。通过引入二分查找思想，将搜索空间按 $16 \rightarrow 8 \rightarrow 4 \rightarrow 2 \rightarrow 1$ 的层级递减，实现了 $O(\log n)$ 复杂度的高效查找。具体实现中，利用条件右移与累加机制逐步缩小搜索范围，最终通过 `b16 + b8 + b4 + b2 + b1 + b0` 的组合计算得到目标位位置。该方法将 `howManyBits` 的操作数控制在32个，远低于90个的上限，体现了算法优化在受限环境下的重要性。

实验中遇到的技术障碍主要通过网络资源获得解决。针对浮点数舍入规则的具体实现、符号扩展的边界条件处理以及溢出检测的判断逻辑等问题，通过检索IEEE 754标准文档、相关技术博客以及开源代码示例，明确了向偶数舍入的位级判断条件、负数除法的偏置量计算方法等关键细节。网络检索过程中需注意信息来源的权威性与适用性，优先参考标准文档与学术资料，避免错误方案的引入。

掩码技术在多个函数中发挥了核心作用。通过构造动态掩码实现条件选择（如 `conditional` 函数中的路由器设计）、位域提取（如 `getByte` 函数中的字节分离）以及符号处理（如 `dividePower2` 函数中的偏置量控制），掩码与逻辑运算的组合有效替代了条件分支语句。特别是在 `fitsBits` 函数中，利用符号扩展的性质，通过 `(!extended) | (!(~extended))` 判断高位是否全0或全1，实现了位宽检测的简洁表达。

浮点数处理部分要求在IEEE 754标准框架下实现数值转换与运算操作。`floatScale1d2` 函数需区分非规格化数、规格化数以及特殊值（NaN与无穷大）等五种情况，其中指数为1的规格化数需转换为非规格化数处理，涉及隐含1位的显式化与尾数的右移舍入。`floatFloat2Int` 函数则需处理指数范围检测、位移方向判断以及溢出边界判定，确保转换结果符合补码整数的表示范围。这些实现过程加深了对浮点数内部结构的认知。

实验的操作数限制与运算符约束促使代码设计趋向最小化与优化。通过移位操作构造大常量（如 `0x01 << 31` 生成 `0x80000000`）、利用算术特性简化运算（如 `~x + 1` 实现取负）以及并行处理技术减少迭代次数（如 `bitCount` 函数的字节并行统计），实验验证了位运算在计算效率与代码简洁性之间的平衡能力。
